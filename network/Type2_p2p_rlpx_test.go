...
var eip8HandshakeAuthTests = []handshakeAuthTest{
	...
	// (Auth©ý) RLPx v4 EIP-8 encoding with version 56, additional list elements
	{
		input: `
...
var eip8HandshakeRespTests = []handshakeAckTest{
	...
	// (Ack©ý) EIP-8 encoding with version 57, additional list elements
	{
		input: `
...
func TestHandshakeForwardCompatibility(t *testing.T) {
	...
	// check derivation for (Auth©ü, Ack©ü) on recipient side
	var (
		hs = &encHandshake{
			initiator:     false,
			respNonce:     nonceB,
